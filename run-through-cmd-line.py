...
if __name__ == '__main__':
    ...
    # Display ML Output with probabilities
    print("\n" + "="*60)
    print("🤖 ML MODEL PREDICTION RESULTS")
    print("="*60)
    print(f"\n📹 Video Processed: {input_video_path}")
    print(f"🎬 Total Frames Detected: {len(out_np_array)}")
    print("\n📊 Prediction Probabilities:")
    for i, action in enumerate(actions):
        prob = prediction[0][i] * 100
        bar = "█" * int(prob / 2)
        print(f"   {action:15s}: {prob:5.2f}% {bar}")
    
    print("\n" + "="*60)
    print(f"✅ PREDICTED ACTION: {actions[arg_pred[0]]}")
    print("="*60 + "\n")
